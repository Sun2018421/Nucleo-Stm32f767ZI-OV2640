...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance==USART1)//如果是串口1，进行串口1 MSP初始化
	{
		...
		GPIO_Initure.Pin=GPIO_PIN_14|GPIO_PIN_15;			//PB14 and PB15
		GPIO_Initure.Mode=GPIO_MODE_AF_PP;		//复用推挽输出
		GPIO_Initure.Pull=GPIO_NOPULL;			//上拉 -->无上拉
		GPIO_Initure.Speed=GPIO_SPEED_FREQ_VERY_HIGH;		//高速-->Very HigH
		GPIO_Initure.Alternate= GPIO_AF4_USART1;	//复用为USART1
		HAL_GPIO_Init(GPIOB,&GPIO_Initure);	   	//初始化PB14 + PB15
#if EN_USART1_RX
